{ ... }:
{
  flake.modules.nixos.buildroot = { config, lib, pkgs, ... }: let
  ...
  in {
    ...
    config = lib.mkIf cfg.enable {
      environment.systemPackages = with pkgs; [
        gnumake gcc binutils patch gzip bzip2 perl cpio unzip rsync wget curl bc file which
        ncurses pkg-config flex bison
        python3 openssl dtc ubootTools
      ];
    };
  };
}
