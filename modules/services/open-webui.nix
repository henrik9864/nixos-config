{ ... }:
{
  flake.modules.nixos.openWebui = { config, lib, pkgs, pkgs-unstable ? pkgs, ... }: let
  ...
  in {
    ...
    config = lib.mkIf cfg.enable {
      assertions = [
        { assertion = config.services.llm.enable; message = "services.openWebui requires services.llm"; }
      ];
