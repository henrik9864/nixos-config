  ...
  in {
    options.services.docker = {
      ...
      storageDriver = lib.mkOption {
        ...
        default = null;
        description = "Docker storage driver to use (e.g. \"overlay2\").";
        example = "overlay2";
      };
      autoPrune = {
        enable = lib.mkOption {
    ...
    config = lib.mkIf cfg.enable {
      ...
      environment.systemPackages = lib.mkIf cfg.enableCompose [ pkgs.docker-compose ];
      users.users = lib.mkMerge (
        map (user: { ${user}.extraGroups = [ "docker" ]; }) cfg.extraUsers
      );
    };
  };
}
