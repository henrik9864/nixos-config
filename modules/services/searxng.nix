{ ... }:
{
  flake.modules.nixos.searxng = { config, lib, pkgs, pkgs-unstable ? pkgs, ... }: let
    cfg = config.services.searxng;
    settingsFormat = pkgs.formats.yaml {};
    defaultSettings = {
      use_default_settings = true;
      general = { debug = false; instance_name = "SearXNG"; };
      server = {
      ...
      search = { safe_search = 0; autocomplete = ""; default_lang = ""; };
      settings.engines = [ { name = "wikidata"; disabled = true; } ];
      enabled_plugins = [
        "Basic Calculator"
        "Hash plugin"
        "Self Information"
  ...
  in {
    options.services.searxng = {
      ...
      secretKeyFile = lib.mkOption {
        ...
        default = null;
        description = "Path to a file containing the secret key used to encrypt cookies.";
      };
      settings = lib.mkOption {
        type = lib.types.attrsOf lib.types.anything;
        default = {};
        description = "SearXNG settings to merge on top of the defaults.";
        example = lib.literalExpression ''
          { general.instance_name = "My Search"; search.safe_search = 1; }
        '';
      };
