{ ... }:
{
  flake.modules.nixos.llm = { config, lib, pkgs, pkgs-unstable ? pkgs, ... }: let
    ...
    cudaArchMap = {
      ...
      "a100" = "80"; "a6000" = "86"; "h100" = "90";
    };

    cudaArch = cudaArchMap.${cfg.gpuModel} or (
    ...
    backendCmakeFlags =
      if cfg.acceleration == "cuda" then [
        "-DGGML_CUDA=ON" "-DGGML_CUDA_ARCHITECTURES=${cudaArch}"
        "-DGGML_VULKAN=OFF" "-DGGML_ROCM=OFF" "-DGGML_METAL=OFF"
      ]
      else if cfg.acceleration == "rocm" then [
        "-DGGML_CUDA=OFF" "-DGGML_ROCM=ON" "-DGGML_VULKAN=OFF" "-DGGML_METAL=OFF"
      ]
      else if cfg.acceleration == "vulkan" then [
        "-DGGML_CUDA=OFF" "-DGGML_ROCM=OFF" "-DGGML_VULKAN=ON" "-DGGML_METAL=OFF"
      ]
      else [
        "-DGGML_CUDA=OFF" "-DGGML_ROCM=OFF" "-DGGML_VULKAN=OFF" "-DGGML_METAL=OFF"
      ];
  ...
  in {
    options.services.llm = {
      ...
      acceleration = lib.mkOption {
        type = lib.types.nullOr (lib.types.enum [ "rocm" "cuda" "vulkan" "metal" ]);
        default = "cuda";
      ...
      gpuModel = lib.mkOption {
        type = lib.types.str;
        default = "rtx-3090";
        description = "GPU model slug for CUDA compute capability.";
        example = "rtx-4090";
      };
      host = lib.mkOption {
        type = lib.types.str;
      ...
      models = lib.mkOption {
        type = lib.types.attrsOf (lib.types.submodule {
          options = {
            enable = lib.mkEnableOption "this model" // { default = true; };
            path = lib.mkOption { type = lib.types.str; description = "Model filename, relative to modelsDir."; };
            port = lib.mkOption { type = lib.types.port; default = 8080; description = "Port this model's server listens on."; };
            contextSize = lib.mkOption { type = lib.types.int; default = 4096; description = "Context window size in tokens."; };
            batchSize = lib.mkOption { type = lib.types.int; default = 2048; description = "Logical batch size."; };
            ubatchSize = lib.mkOption { type = lib.types.int; default = 512; description = "Physical (micro) batch size."; };
            gpuLayers = lib.mkOption { type = lib.types.int; default = 99; description = "Number of layers to offload to GPU."; };
            flashAttn = lib.mkOption { type = lib.types.bool; default = false; description = "Enable flash attention."; };
            mtp = {
              enable = lib.mkEnableOption "MTP speculative decoding";
              draftTokens = lib.mkOption { type = lib.types.int; default = 4; description = "Speculative draft tokens per step."; };
            };
            extraArgs = lib.mkOption { type = lib.types.listOf lib.types.str; default = []; description = "Extra arguments for llama-server."; };
          };
        });
        default = {};
        description = "Named models, each served on its own port.";
      };
      common.extraArgs = lib.mkOption {
    ...
    config = lib.mkIf cfg.enable (let
      effectiveModels = lib.filterAttrs (_: v: v.enable) cfg.models;
    in {
      assertions = [
        { assertion = effectiveModels != {}; message = "services.llm.enable is true but no models are defined in services.llm.models."; }
      ];
